...
#pragma HLS unroll
                data_out.max[i >> 1] = (data_in.max[i] > data_in.max[i + 1]) ? data_in.max[i] : data_in.max[i + 1];
            }
            for (int i = 0; i < 16; i += 2) {
#pragma HLS unroll
                data_out.min[i >> 1] = (data_in.min[i] < data_in.min[i + 1]) ? data_in.min[i] : data_in.min[i + 1];
            }
            for (int i = 0; i < 16; i += 2) {
#pragma HLS unroll
