...
#pragma HLS array_partition variable=sum_buffer complete 
    while (1) {
        ...
        if (app_stat_writer_input.read_nb(data_in)) {
            max_buffer[read_cnt] = (data_in.max[0] > data_in.max[1]) ? data_in.max[0] : data_in.max[1];         
            min_buffer[read_cnt] = (data_in.min[0] < data_in.min[1]) ? data_in.min[0] : data_in.min[1];
            sum_buffer[read_cnt] = (data_in.sum[0] + data_in.sum[1]); 

            app_data_out.data(128 * read_cnt + 31, 128 * read_cnt) = max_buffer[read_cnt]; app_data_out.data(128 * read_cnt+ 63, 128 * read_cnt + 32) = min_buffer[read_cnt];
            app_data_out.data(128 * read_cnt + 127, 128 * read_cnt + 64) = sum_buffer[read_cnt];
            read_cnt ++;
            if (read_cnt == 4) {
