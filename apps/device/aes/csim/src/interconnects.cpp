...
/*
 * refer to https://blog.csdn.net/qq_28205153/article/details/55798628
 * and
...
int main() {
  ...
  std::thread t22(
      ...
      std::ref(reset_sigs), std::ref(reset_dram_helper_app),
      std::ref(reset_pcie_helper_app), std::ref(reset_pcie_data_splitter_app));
  std::thread t23(app_aes_start, std::ref(reset_app_aes_start),
  ...
  std::thread t32(app_aes_process_8, std::ref(reset_app_aes_process_8),
                  std::ref(app_aes_proc_8_params), std::ref(app_aes_end_params),
                  std::ref(app_aes_proc_8_input_data),
                  std::ref(app_aes_end_input_data));
  std::thread t33(app_aes_end, std::ref(reset_app_aes_end),
                  std::ref(app_aes_end_params),
                  std::ref(app_aes_end_input_data), std::ref(app_output_data));
  std::thread t34(host_pcie_simulator);
  std::thread t35(fpga_dramA_simulator);
  std::thread t36(fpga_dramB_simulator);
  std::thread t37(fpga_dramC_simulator);
  std::thread t38(fpga_dramD_simulator);
