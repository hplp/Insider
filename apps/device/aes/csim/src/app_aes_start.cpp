...
void app_aes_start(ST_Queue<bool> &reset_app_aes_start,
...
#pragma HLS array_partition variable = s_table complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_aes_start.read_nb(dummy))) {
      valid_param = false;
      input_param_num = 0;
...
#pragma HLS pipeline
      ...
      APP_Data data_in;
      APP_Data data_out;
      if ((!valid_param) && app_input_param.read_nb(param)) {
        // param for subbytes
        ...
        input_param_num++;
        app_aes_proc_0_params.write(param);
      } else if (valid_param && app_input_data.read_nb(data_in)) {
        for (int i = 0; i < 4; i++) {
#pragma HLS unroll
        ...
        data_out.len = 64;
        app_aes_proc_0_input_data.write(data_out);
      }
    }
  }
}
