...
void app_aes_process_4(ST_Queue<bool> &reset_app_aes_process_4,
...
#pragma HLS array_partition variable = s_table complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_aes_process_4.read_nb(dummy))) {
      valid_param = false;
      input_param_num = 0;
      ...
      key_table = Matrix();
      unsigned int dummy0;
      app_aes_proc_4_params.read_nb(dummy0);
      struct APP_Data dummy1;
      app_aes_proc_4_input_data.read_nb(dummy1);
      reset_cnt++;
...
#pragma HLS pipeline
      ...
      APP_Data data_in;
      APP_Data data_out;
      if ((!valid_param) && app_aes_proc_4_params.read_nb(param)) {
        // param for subbytes
      ...
      } else if (valid_param && app_aes_proc_4_input_data.read_nb(data_in)) {
        for (int i = 0; i < 4; i++) {
#pragma HLS unroll
