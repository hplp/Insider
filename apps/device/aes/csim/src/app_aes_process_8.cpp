...
void app_aes_process_8(ST_Queue<bool> &reset_app_aes_process_8,
...
#pragma HLS array_partition variable = s_table complete dim = 0
  ...
  while (1) {
    bool dummy;
    if (reset || (reset = reset_app_aes_process_8.read_nb(dummy))) {
      valid_param = false;
      input_param_num = 0;
      ...
      key_table = Matrix();
      unsigned int dummy0;
      app_aes_proc_8_params.read_nb(dummy0);
      struct APP_Data dummy1;
      app_aes_proc_8_input_data.read_nb(dummy1);
      reset_cnt++;
