...
unsigned char S_matrix[16][16] = {
    ...
    0xb0, 0x54, 0xbb, 0x16};
unsigned char mix_matrix[4][4] = {0x02, 0x03, 0x01, 0x01, 0x01, 0x02,
...
unsigned char host_GFMul3(unsigned char a) { return host_GFMul2(a) ^ a; }

unsigned char host_GFMul(unsigned char a, unsigned char b) {
  if (a == 1)
    return b;
  else if (a == 2)
    return host_GFMul2(b);
  else if (a == 3)
    return host_GFMul3(b);
}
...
void user_simulation_function() {
  ...
  puts("expected result generated...");
  ...
  while (!fin_file) {
    ...
    printf("INFO: total read bytes: %d\n", read_bytes);
