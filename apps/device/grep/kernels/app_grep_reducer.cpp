    ...
    while (1) {
        ...
        if (app_grep_matching_vecs.read_nb(matching_vec)) {
            ...
            if (!matching_vec.valid && matching_vec.eop) {
                ...
                int col_indices[(MATCHING_VEC_SIZE + COMPARE_CHUNK_SIZE - 1) / COMPARE_CHUNK_SIZE];
#pragma HLS array_partition variable=col_indices complete dim=1

                for (int i = 0; i < (MATCHING_VEC_SIZE + COMPARE_CHUNK_SIZE - 1) / COMPARE_CHUNK_SIZE; i ++) {
#pragma HLS unroll
