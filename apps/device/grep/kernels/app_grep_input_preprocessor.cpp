...
#pragma HLS unroll
                grep_input_data.col_indices[i] = -MAX_GREP_PARAM_SIZE + 1 + (col_state_cnt << DATA_BUS_WIDTH_LOG2) + i;
                if (grep_input_data.col_indices[i] >= 0) {
                    if (i < MAX_GREP_PARAM_SIZE - 1) {
                        grep_input_data.data[i] = prev_input_data_str[i];
                    }
                    else {	
                        grep_input_data.data[i] = cur_input_data_in_bytes[i - (MAX_GREP_PARAM_SIZE - 1)];
                    }
                }
                else {
...
#pragma HLS unroll
                prev_input_data_str[i] = cur_input_data_in_bytes[DATA_BUS_WIDTH - (MAX_GREP_PARAM_SIZE - 1) + i];
            }
