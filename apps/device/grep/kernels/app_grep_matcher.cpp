...
#pragma HLS unroll
                    grep_param_str[i + loading_grep_param_str_index] = (input_param >> ((POKE_WIDTH_BYTES - 1 - i) << 3)) & 0xFF;
                }
...
#pragma HLS unroll
                    for (int j = 0; j < MAX_GREP_PARAM_SIZE; j ++) {
                        if (j < grep_param_size && i + j < MATCHING_VEC_SIZE) {
                            matched_str[i] &= (grep_param_str[j] == grep_input_data.data[i + j]);
                        }
                    }
                }
...
#pragma HLS unroll
                ...
                for (int i = MATCHING_VEC_SIZE; i < MATCHING_VEC_SIZE + COMPARE_CHUNK_SIZE - 1; i ++) {
#pragma HLS unroll
