...
void app_sql_writer (
        ST_Queue<SQL_Record> &app_sql_filter0_output_records, 
        ...
        if (valid_record0 && valid_record1) {
            ...
            if (has_full_write) {
                ...
                for (int i = 0; i < 2; i ++) {
                    ...
                    output_app_data_in_bytes[32 + 12 + 12 + 2 + i] = output_record1.month[i];
                }
                for (int i = 0; i < 2; i ++) {
                    output_app_data_in_bytes[i + 12 + 12 + 2 + 2] = output_record0.day[i];
                    output_app_data_in_bytes[32 + 12 + 12 + 2 + 2 + i] = output_record1.day[i];
                }
                for (int i = 0; i < 2; i ++) {
                    output_app_data_in_bytes[i + 12 + 12 + 2 + 2 + 2] = output_record0.year[i];
                    output_app_data_in_bytes[32 + 12 + 12 + 2 + 2 + 2 + i] = output_record1.year[i];
                }
                output_app_data.eop = output_record0.eop | output_record1.eop;
                output_app_data.len = 32 * (output_record0.valid + output_record1.valid);
                for(int idx = 0; idx < 512; idx +=  8) {
                    output_app_data.data(idx + 7, idx) = output_app_data_in_bytes[idx / 8];
                }
            ...
            if (lefting_record.eop) {
                ...
                for (int i = 0; i < 2; i ++) {
                    output_app_data_in_bytes[i + 12 + 12 + 2 + 2 + 2] = lefting_record.year[i];
