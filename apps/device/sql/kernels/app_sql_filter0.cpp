#ifndef APP_SQL_FILTER0_CPP_
#define APP_SQL_FILTER0_CPP_
#include <insider_kernel.h>

#include "constant.h"
#include "structure.h"

void app_sql_filter0 (
    ...
    while (1) {
        if (!valid_filter_params) {
            ...
            if (app_sql_filter0_params.read_nb(filter_params)) {
            ...
            SQL_Record record;
            if (app_sql_filter0_input_records.read_nb(record)) {
                unsigned char year = (record.year[1] - '0') + (record.year[0] - '0') * 10;
                record.valid = (year >= year_lower_thresh && year <= year_upper_thresh);
                app_sql_filter0_output_records.write(record);
            }
        }
    }
