    ...
    while (1) {
        ...
        if (!valid_predicting_vec) {
            if (app_input_params.read_nb(input_param)) {
                predicting_vec[received_predicting_vec_idx >> 6][received_predicting_vec_idx & 63] = input_param;
            ...
            if (app_input_data.read_nb(input_app_data)) {
                if (cnt_input_buf == 0) {
                    cur_result = (input_app_data.data(503, 496) - '0') * 10 + (input_app_data.data(511, 504) - '0');
                }
                else {
...
#pragma HLS unroll
                    ...
                    for (int i = 0; i < 64; i ++) {
#pragma HLS unroll
                        char diff = (char)predicting_vec[predicting_vec_dim0_idx][i] - (char)weights[i];
                ...
                cnt_input_buf ++;
                if (cnt_input_buf == (RESULT_SIZE + WEIGHT_SIZE * FEATURE_DIM) / DATA_BUS_WIDTH) {
