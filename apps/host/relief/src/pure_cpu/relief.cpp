...
const long long TRAIN_SET_SIZE = (APP_HIT_DATA_NUM + APP_MISS_DATA_NUM) * APP_FEATURE_DIM * sizeof(int);
...
void update_weight() {
    for (int i = 0; i < APP_QUERY_NUM; i ++) {
        for (int j = 0; j < APP_FEATURE_DIM; j ++) {
            weight[j] += ((int) (negVec[i][j] == query[i][j])) - ((int) (posiVec[i][j] == query[i][j]));
        }
    }
    for (int i = 0; i < APP_FEATURE_DIM; i ++) {
...
inline void read_filte() {
    ...
    while (read_cnt != TRAIN_SET_SIZE / READ_BUF_SIZE) {
        ...
        while (read_bytes != READ_BUF_SIZE) {
            ...
            if (curReadBytes == 0) break; 
            read_bytes += curReadBytes;
        }
        read_cnt ++;
        int *read_buf_ptr_in_4B = (int *) read_buf;
        cout << "total read bytes is: " << read_bytes << endl;
        for (int i = 0; i < read_bytes / APP_FEATURE_DIM / sizeof(int) / 2; i ++) {  // should change if multiple time read
            cout << i << endl;
...
void read_train(void) {
    ...
    while (read_cnt !=  TRAIN_SET_SIZE / READ_BUF_SIZE) {
        ...
        for (int i = 0; i < read_bytes / APP_FEATURE_DIM / sizeof(int) / 2; i ++) { 
            ...
            find_nearest(read_buf_ptr_in_4B, dist, (read_cnt < (TRAIN_SET_SIZE >> 1)));
            read_buf_ptr_in_4B += APP_FEATURE_DIM;
        }
    }
    update_weight();
    cout << "DEBUG: Expected data size is: " << validFeatureNum * (APP_HIT_DATA_NUM + APP_MISS_DATA_NUM) * sizeof(int) << endl;
