...
unsigned char feature_vec[NUM_TRAIN_CASES][FEATURE_DIM], predicting_vec[NUM_PREDICTING_CASES][FEATURE_DIM];
...
void read_data(void) {
  int fd = open("/mnt/train_data.txt", O_RDWR);
  ...
  while (1) {
    int read_len = 0;
    int tmp_len;
    while (read_len != READ_BUF_SIZE && (tmp_len = read(fd, buf, READ_BUF_SIZE - read_len))) {
      read_len += tmp_len;
    }
    if (!tmp_len) {
      break;
    ...
    for (k = 0; k < READ_BUF_SIZE / BUS_WIDTH; k ++) {
      ...
      if (state == 0) {
	// result
	result[data_idx] = c2i(buf_ptr[BUS_WIDTH - 2]) * 10 + c2i(buf_ptr[BUS_WIDTH - 1]);
	state ++;
      }
      else {
	// feature
	for (i = 0; i < BUS_WIDTH / WEIGHT_SIZE; i ++) {
	  ...
	  for (int j = 0; j < WEIGHT_SIZE; j ++) {
	    weight += c2i(buf_ptr[buf_idx + j]) * ten_exp_table[WEIGHT_SIZE - 1 - j];
	  }
