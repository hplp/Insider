...
inline bool verify(const char *q, const char *x) {
    ...
    cout << "similarity: " << similarity  << " = " << overlap << "/" << APP_QUERY_LENGTH << endl;
#endif    
...
inline int prefix_filter(const char *query, const char *dataset, const int &dataset_len) {
    bool match = false;
    bool similar;
    int match_pairs = 0;
...
void read_data(const char *query) {
    ...
    cout << "total number of matches is: " << num_of_matches << " (" <<  APP_RECORD_NUM << " in total)" <<endl;
}
char random_char() {
    int min_ascii = 65;
    int max_ascii = 69;
