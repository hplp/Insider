...
inline void calculation(int readBytes) {
    ...
    for (int i = 0; i < readBytes / sizeof(unsigned int) / 4; i ++) {
        ...
        unsigned long long v_avg = ((((unsigned long long)v_sum_h) << 32 ) | v_sum_l) / (COL_NUM / sizeof(unsigned int));
        total += (v_max + v_min + v_avg);
        //cout << "v_max = " << v_max << ", v_min = " << v_min << " , v_avg = " << v_avg << endl;
    }
}
...
void work(void) {
    ...
    while (!fin) {
        ...
        while (readBytes != READ_BUF_SIZE) {
            int tmp = iread(fd, &buf, READ_BUF_SIZE - readBytes);
            if (tmp) {
                readBytes += tmp;
            }
            else {
