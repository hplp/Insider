...
#pragma HLS array_partition variable=maxMissVec cyclic factor=16 dim=2
  int tmpVec[APP_FEATURE_PER_CYCLE][APP_FEATURE_DIM >> APP_FEATURE_PER_CYCLE_LOG2];
...
#pragma HLS array_partition variable=diff_out.maxMissVec complete dim=0
    if (app_rlf_stg_input_data.read_nb(stg_in)) {
      char idx = stg_in.idx;
      char updateCnt = (idx + 1) & ((APP_FEATURE_DIM >> APP_FEATURE_PER_CYCLE_LOG2) - 1);
      int tmp;
      for (int j = 0; j < APP_FEATURE_PER_CYCLE; j++) {
#pragma HLS unroll
	if (idx == (APP_FEATURE_DIM >> APP_FEATURE_PER_CYCLE_LOG2) - 1) {
	  tmpVec[j][(APP_FEATURE_DIM >> APP_FEATURE_PER_CYCLE_LOG2) - 1] = stg_in.data[j];
	  tmp = tmpVec[j][0]; 
	}
	else {
...
#pragma HLS unroll              
	for (int j = 0; j < APP_FEATURE_PER_CYCLE; j++) {
#pragma HLS unroll
	  diff_out.maxHitVec[j] = maxHitVec[i][APP_FEATURE_PER_CYCLE * writeCnt + j];
	  diff_out.maxMissVec[j] = maxMissVec[i][APP_FEATURE_PER_CYCLE * writeCnt + j];
	}
      }
