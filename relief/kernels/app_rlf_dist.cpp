...
#pragma HLS array_partition variable=query cyclic factor=16 dim=2 
  ...
  while (1) {
	...
	query[readParamCnt >> APP_FEATURE_DIM_LOG][readParamCnt & (APP_FEATURE_DIM - 1)] = (int) param;
	readParamCnt++;
...
#pragma HLS unroll
	    ...
	    reduce16_out.reduce[j] = (dist_in.data[j] - query[i][APP_FEATURE_PER_CYCLE * idx + j]) * (dist_in.data[j] - query[i][APP_FEATURE_PER_CYCLE * idx + j]);
	  }
